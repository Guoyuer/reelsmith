...
def _make_click_wav(path: Path, bpm: int = 120, duration_s: float = 10.0, sample_rate: int = 44100) -> Path:
...
def _make_silence_wav(path: Path, duration_s: float = 5.0, sample_rate: int = 48000) -> Path:
...
def _make_test_video(path: Path, duration: float = 3.0, size="320x180", fps: int = 24, audio: bool = False) -> Path:
    ...
    if audio:
        cmd += ["-f", "lavfi", "-i", f"sine=frequency=440:duration={duration}:sample_rate=48000"]
        ...
        edl = EDL(
            ...
            segments=[
                Segment(
                    name="Speech",
                    items=[
                        EditItem(source_file="a.mp4", media_type="video", display_duration=4.0, keep_audio=True),
                        EditItem(source_file="b.mp4", media_type="video", display_duration=4.0, keep_audio=True),
                    ],
                    transition="crossfade",
                    transition_duration=0.5,
                ),
        ...
        edl = EDL(
            ...
            segments=[
                Segment(
                    name="Segment A",
                    items=[
                        EditItem(source_file=str(photos[0]), media_type="photo", display_duration=3.0, effect="static"),
                        EditItem(source_file=str(photos[1]), media_type="photo", display_duration=3.0, effect="static"),
                    ],
                    transition="crossfade",
                    transition_duration=0.5,
                ),
                Segment(
                    name="Segment B",
                    items=[
                        EditItem(source_file=str(photos[2]), media_type="photo", display_duration=3.0, effect="static"),
                    ],
        ...
        edl = EDL(
            ...
            segments=[
                Segment(
                    name="Only",
                    items=[
                        EditItem(source_file=str(photo), media_type="photo", display_duration=3.0, effect="static"),
                    ],
                    transition="cut",
                    transition_duration=0.0,
                ),
...
class TestXfadeConcatenation:
    ...
    def test_three_clips_xfade(self, tmp_path):
        from pipeline.assemble._concat import concat_xfade as _concat_xfade
        from pipeline.media_utils import probe_duration as _probe_duration
        ...
        for i, color in enumerate(["red", "green", "blue"]):
            ...
            subprocess.run(
                [
                    ...
                    "lavfi",
                    "-i",
                    f"color=c={color}:s=320x180:r=24:d=3",
                    "-c:v",
                    "libx264",
                    "-preset",
                    "ultrafast",
    ...
    def test_group_splitting_12_clips(self, tmp_path):
        ...
        from pipeline.assemble._concat import concatenate as _concatenate
        from pipeline.media_utils import probe_duration as _probe_duration
...
class TestSpeechTrackBuild:
    def test_speech_at_offset(self, tmp_path):
        ...
        from pipeline.assemble._audio import build_speech_track as _build_speech_track
        from pipeline.media_utils import probe_duration as _probe_duration
...
class TestAddMusic:
    ...
    def test_music_with_speech_ducking(self, tmp_path):
        ...
        _add_music(video, music, output, speech_ranges=[(2.0, 5.0)], speech_audio_path=speech_wav)

            ...
            for item in seg.items:
                if item.media_type == "video" and item.start_time is not None:
                    vid_dur = analysis_by_id.get(
                        next(
                            (aid for aid, a in analysis_by_id.items() if a.get("local_path") == item.source_file), None
                        ),
                        {},
                    ).get("video_duration")
...
class TestVideoEffectOverride:
    ...
    def test_video_ken_burns_forced_to_none(self):
        edl = _make_test_edl(
            [
                EditItem(source_file="a.mp4", media_type="video", display_duration=5.0, effect="ken_burns_in"),
            ]
        )
        for seg in edl.segments:
            for item in seg.items:
    ...
    def test_photo_effect_preserved(self):
        edl = _make_test_edl(
            [
                EditItem(source_file="a.jpg", media_type="photo", display_duration=4.0, effect="ken_burns_left"),
            ]
        )
        for seg in edl.segments:
            for item in seg.items:
